...
func BuildDicTrie(cfg *Config) {
	var fillerbyte [][]byte
	var finderbyte [][]byte
	for _, filler := range cfg.FillterList {
		fillerbyte = append(fillerbyte, []byte(filler))
	}
	for _, finder := range cfg.FindList {
		finderbyte = append(finderbyte, []byte(finder))
	}
	InitProduceNode(fillerbyte, "fillter_list")
	InitProduceNode(finderbyte, "find_list")
}

func InitProduceNode(byteData [][]byte, name string) {
	var tree Trie
	tree.InitRootNode()
	tree.BuildTrie(byteData)
	dicFileTrie[name] = &tree
	dicFileTime[name] = time.Now().Unix()
}
...
func (s keyTMP) Len() int {
	return len(s)
}
func (s keyTMP) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}
func (s keyTMP) Less(i, j int) bool {
	return s[i] < s[j]
}
...
func MatchText(dicFile string, text []rune, seps Seps, rep rune) string {
	T := dicFileTrie[dicFile]
	if T == nil {
		return ""
	}
	sort.Sort(seps)
	walkNode := T.RootNode
	var nextNode *Node
	for i, charactor := range text {
		for {
			if FindSepC(seps, charactor) {
				break
			}
			nextNode = walkNode.BinGetChildNodeByVal(charactor)
			if nextNode == nil {
				if nil != walkNode.SuffixNode {
					walkNode = walkNode.SuffixNode
					continue
				} else {
					walkNode = T.RootNode
				...
				if nextNode.EOW {
					...
					j := i
					var keyData keyTMP
					var keySortData string
					keyTmpData := make(map[int]string)
					for depth > 0 {
						for j >= 0 && FindSepC(seps, text[j]) {
							j--
						}
						if j >= 0 {
					...
					sort.Sort(keyData)
					for _, line := range keyData {
						keySortData = keySortData + keyTmpData[line]
					}
					walkNode = T.RootNode
					return string(keySortData)
				} else {
					walkNode = nextNode
				}
				break
